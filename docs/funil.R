funil <- function (x, text = NULL, color = NULL) 
{
  if (!is.numeric(x)) {
  ...
  for (i in 1:length(dfs)) {
    p <- p + ggplot2::geom_polygon(data = dfs[[i]], ggplot2::aes(x = x, 
                                                                 y = y), fill = color[i]) + ggplot2::geom_text() + 
      ggplot2::annotate("text", label = text[i], x = x[1]/2, 
                        y = mean(dfs[[i]]$y), size = log10(x[i])/ggplot2::.pt) + 
      ggplot2::theme_minimal() + ggplot2::theme(axis.title.x = ggplot2::element_blank(), 
